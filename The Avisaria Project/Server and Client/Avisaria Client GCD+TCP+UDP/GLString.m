...
- (void) genTexture; // generates the texture without drawing texture to current context
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 4, texSize.width, texSize.height,
				 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8_REV, [bitmap bitmapData]);
...
- (void) drawWithBounds:(NSRect)bounds
{
	...
	if (texName) {
		...
		glBegin (GL_QUADS);
			...
			glTexCoord2f(0.0f,0.0f); glVertex2f( bounds.origin.x,  bounds.origin.y);   // bottom left
			glTexCoord2f(1.0f,0.0f); glVertex2f( bounds.size.width+bounds.origin.x,  bounds.origin.y);   // bottom right
			glTexCoord2f(1.0f,1.0f); glVertex2f( bounds.size.width+bounds.origin.x, bounds.size.height+bounds.origin.y);   // top right
			glTexCoord2f(0.0f,1.0f); glVertex2f( bounds.origin.x,  bounds.size.height+bounds.origin.y);   // top left
