...
- (void)socket:(GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port
{
	...
	NSError *error = nil;
	if(![listenUdpSocket connectToHost:host onPort:port error:&error])
	{
		[delegate logMessage: [NSString stringWithFormat:@"Error starting UDP client: %@", error]];
	} else {
		[delegate logMessage: [NSString stringWithFormat:@"Accepted UDP server %@:%hu", host, port]];
	}
	udpID = 0;
...
- (void)sendUdpData:(NSData*)data {
	[listenUdpSocket sendData:data withTimeout:0.1 tag:0];
}
