...
@synthesize tiles,selectedTile,scroller;
...
- (void)awakeFromNib
{
	...
	[scroller setAction:@selector(scrolled:)];
	porportion = ([tiles count])/2*88;
	porportion = screenDimensions.y/porportion;
	[scroller setFloatValue:scroll knobProportion:porportion];
...
{
	int multValue = 88*round([tiles count]/2.0) - screenDimensions.y;
    switch ([scroller hitPart]) {
        case NSScrollerIncrementLine:
			// Include code here for the case where the down arrow is pressed
			if (scroll < 1.0) {
				scroll += 0.1*multValue;
			}
            break;
        ...
        case NSScrollerKnob:
            ...
            scroll = [scroller floatValue]*multValue;
	...
	[scroller setFloatValue:scroll/multValue knobProportion:porportion];
...
- (void) handleGameTimer: (NSTimer *) gameTimer {
	...
	porportion = screenDimensions.y/porportion;
	[scroller setFloatValue:scroll knobProportion:porportion];
	if (porportion >= 1.0) {[scroller setEnabled:FALSE];}
	if (updateScreen) {
...
- (void)mouseDown:(NSEvent*)theEvent
{
	...
	selectedTile = floor((aMousePoint.y+ scroll)/88)*floor(screenDimensions.x/88) + floor(aMousePoint.x/88);
	if (selectedTile > [tiles count] - 1) {selectedTile = 0;}
	selectedTilePos = CGPointMake(floor(aMousePoint.x/88),floor((aMousePoint.y + scroll)/88));
	updateScreen = TRUE;
}
...
-(void) setTile:(int)tile {
	selectedTile = tile;
	selectedTilePos = CGPointMake((tile/(screenDimensions.x/88) - floor(tile/(screenDimensions.x/88)))*(screenDimensions.x/88),floor(tile/(screenDimensions.x/88)));
	scroll = selectedTilePos.y*88;
	updateScreen = TRUE;
}
...
- (void)drawRect:(NSRect)theRect {
	...
	if (tiles) {
		...
		for (int i = 0;i < [tiles count]; i++) {
			if (x == floor(screenDimensions.x/88)) {
				y += 1;
				x = 0;
			}
			if (y*88-scroll > -88 && y*88-scroll < screenDimensions.y) {
			[self drawImage:context translate:CGPointMake(0, 0) image:[tiles objectAtIndex:i] point:CGPointMake(x*88, round(y*88 - scroll)) rotation:0.0];
			}
				x += 1;
			}
	}
	[self drawRectangle:context translate:CGPointMake(0, 0) point:CGPointMake(selectedTilePos.x * 88, round(selectedTilePos.y * 88-scroll)) widthheight:CGPointMake(88, 88) color:color rotation:0.0 filled:FALSE linesize:1.0];
}
